...
export const neutralTheme = defineTheme({
  ...
  // Typography: Be Vietnam Pro across body, heading, and display sizes (display
  // size tokens inherit from heading.family). Monospace stays as the
  // platform default for code.
  //
  // Substituted for the stock Figtree: this site is entirely Vietnamese, and
  // Figtree ships no Vietnamese subset (latin + latin-ext only), so every
  // diacritic (ế ộ ữ ị ằ) would fall back glyph-by-glyph to a different face.
  // Be Vietnam Pro has full Vietnamese coverage and the closest character to
  // Figtree. Loaded via next/font, which exposes --font-be-vietnam-pro.
  // This is the ONLY departure from stock theme-neutral.
  //
