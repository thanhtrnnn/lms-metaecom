...
type Event = {title: string; description?: string | null; date?: string | null};
    ...
    <Section padding={6}>
      <VStack gap={6}>
        <PageHeader
          ...
          current="Webinar"
        />
        <Heading level={2}>{webinars.sectionTitle ?? 'Sự Kiện Sắp Diễn Ra'}</Heading>
        {events.length === 0 ? (
          <EmptyState
          ...
          <VStack gap={3}>
            {events.map((e) => (
              <Card key={e.title} padding={4}>
                <HStack gap={4} vAlign="center" wrap="wrap">
                  <StackItem size="fill">
                    <VStack gap={2}>
                      {e.date ? (
                        <Badge label={e.date} variant="info" />
                      ) : null}
